...
const getColumns = () => [
  ...
  },
  {
    ...
    accessor: "feeStatus",
    Cell: ({ row }) => {
      const feeStatus = row.original.feeStatus; // Access the actual feeStatus value
      const className = feeStatus === "paid" ? "bg-green-300" : "text-amber-300 animate-pulse"; 
      return (
        <td className={`p-3 text-sm font-normal text-gray-700 first:rounded-l-lg last:rounded-r-lg ${className}`}>
          {feeStatus}
        </td>
      );
    },
...
function Table1(students) {
  const data = useMemo(() => generateData(students), []);
